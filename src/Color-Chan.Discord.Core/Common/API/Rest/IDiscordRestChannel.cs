...
namespace Color_Chan.Discord.Core.Common.API.Rest
{
    ...
    public interface IDiscordRestChannel
    {
        ...
        /// <param name="ct">The <see cref="CancellationToken" />.</param>
        /// <returns>
        ///     A <see cref="Result{T}" /> of <see cref="IReadOnlyList{T}"/> of <see cref="IDiscordMessage" /> with the request results.
        /// </returns>
        /// <exception cref="ArgumentOutOfRangeException">Thrown when <paramref name="limit"/> is out of range.</exception>
        Task<Result<IReadOnlyList<IDiscordMessage>>> GetChannelMessagesAsync(ulong channelId, ulong? around = null, ulong? before = null, ulong? after = null, int limit = 50, CancellationToken ct = default);
        ...
        ///         </listheader>
        ///         <item>
        ///             <description>When operating on a guild channel, the current user must have the SEND_MESSAGES permission.</description>
        ///         </item>
        ///         <item>
        ///             <description>When sending a message with tts (text-to-speech) set to true, the current user must have the SEND_TTS_MESSAGES permission.</description>
        ///         </item>
        ///         <item>
        ///             <description>
        ...
        /// <param name="message">The <see cref="DiscordCreateChannelMessage"/> containing the data that will be used to create a message.</param>
        /// <param name="ct">The <see cref="CancellationToken" />.</param>
        /// <returns>
        ///     A <see cref="Result{T}" /> of <see cref="IDiscordMessage" /> with the request results.
        /// </returns>
